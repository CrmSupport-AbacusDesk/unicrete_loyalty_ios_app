...
export class GiftDetailPage {
    ...
    getGiftDetail(gift_id)
    {
        ...
        .subscribe( (r) =>
        {
            ...
            var referral_amount = ((this.karigar_detail.referal_point_balance * referal_point_per)/100);
            this.offer_balance= parseInt(this.karigar_detail.balance_point) + (referral_amount);
            console.log(this.offer_balance);
            
            if(this.offer_balance < this.gift_detail.coupon_points)
            {
                
                this.toastAlert(this.msg_alert);
            }
        });
    }
    presentLoading() 
